//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "WAJSContextPluginBase.h"

@class CADisplayLink, NSMutableArray, NSString, WARemoteDebugMaskView, WARemoteDebugPanelView;

@interface WAJSContextPlugin_RemoteDebug : WAJSContextPluginBase
{
    float _panelWidth;
    NSMutableArray *_printItemList;
    CADisplayLink *_updateTimer;
    WARemoteDebugPanelView *_panelView;
    WARemoteDebugMaskView *_maskView;
    BOOL _isDisplay;
    unsigned long _noConfirmedCount;
    unsigned long _noSendCount;
    unsigned long _sendLength;
    unsigned long _sendCount;
    unsigned long _recvLength;
    unsigned long _recvCount;
    NSString *_connectString;
    NSString *_serverString;
    NSString *_maskString;
}

@property(copy, nonatomic) NSString *maskString; // @synthesize maskString=_maskString;
@property(copy, nonatomic) NSString *serverString; // @synthesize serverString=_serverString;
@property(copy, nonatomic) NSString *connectString; // @synthesize connectString=_connectString;
@property(nonatomic) unsigned long recvCount; // @synthesize recvCount=_recvCount;
@property(nonatomic) unsigned long recvLength; // @synthesize recvLength=_recvLength;
@property(nonatomic) unsigned long sendCount; // @synthesize sendCount=_sendCount;
@property(nonatomic) unsigned long sendLength; // @synthesize sendLength=_sendLength;
@property(nonatomic) unsigned long noSendCount; // @synthesize noSendCount=_noSendCount;
@property(nonatomic) unsigned long noConfirmedCount; // @synthesize noConfirmedCount=_noConfirmedCount;
@property(nonatomic) BOOL isDisplay; // @synthesize isDisplay=_isDisplay;
- (void).cxx_destruct;
- (void)updatePanelView;
- (void)destoryUpdateTimer;
- (void)initUpdateTimer;
- (void)didRotate:(id)arg1;
- (void)consoleLog:(id)arg1 isSendout:(BOOL)arg2;
- (void)onStopButtonChanged:(id)arg1;
- (void)initMaskView;
- (void)initPanelView;
- (BOOL)handleEvent:(int)arg1 userInfo:(id)arg2;
- (id)init;
- (void)dealloc;

@end

